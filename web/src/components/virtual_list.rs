                    ...
                    <div class="flex flex-col p-4 space-y-2 flex-grow md:w-7/12">
...

                        {
                            if episode.completed {
                                ...
                                if  episode.listenduration.clone().unwrap_or_default() > 0 {
                                    let listen_duration_percentage = if episode_duration_in_seconds > 0 {
                                        ((episode.listenduration.clone().unwrap_or_default() as f64 / episode_duration_in_seconds as f64) * 100.0).min(100.0)
                                    } else {
                                        0.0
                                    };
                                    html! {
                                        <div class="flex items-center space-x-2">
                                            // Only show current position in wider viewports
                                            {
                                                if !is_narrow_viewport {
                                                    html! {
                                                        <span class="item_container-text">{ format_time(episode.listenduration.clone().unwrap_or_default() as f64) }</span>
                                                    }
                                                } else {
                                                    html! {}
                                                }
...
pub fn person_episode_component(props: &PersonEpisodeComponentProps) -> Html {
    ...
    let on_play_pause = on_play_pause(
        ...
        audio_state.clone(),
        None,
        false, // person episodes are always non-YouTube
    );
